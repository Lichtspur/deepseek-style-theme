...
` }
		...
		*/
		...
		/**
		* Action menu for the product's delivered-file cards
		* (`@deepseek-ai/dsh-client-ui-deliverables`).
		*
		* The product's card already offers "open with the default app" and
		* "reveal in the file manager", but both hide behind a small chevron, and
		* "copy path" does not exist at all — the absolute path is only reachable
		* by reading the card's `title` tooltip by hand. This patch adds a
		* right-click menu carrying all four gestures, and the missing "copy path"
		* item inside the product's own chevron menu.
		*
		* The card's left click is deliberately left alone: the product wires it to
		* the sidebar preview, and hijacking it would trade a working feature for a
		* menu. `[data-presented-file]` is a stable product attribute rather than a
		* build-hashed class, and the absolute path is already resolved onto the
		* overlay button's `title` by the product's own `resolveWorkspacePath`, so
		* this patch never re-derives a workspace.
		*/
...
`;
			...
			/** The absolute path the product already resolved onto the card. */
			const pathOf = (card) => {
			...
			const overlayOf = (card) => card.querySelector('button[title]');
			...
			const openMenu = (card, clientX, clientY) => {
				closeMenu();
				...
				node.appendChild(buildItem(ICON_PREVIEW, text.preview, () => {
					const overlay = overlayOf(card);
					if (overlay !== null) overlay.click();
				}));
				document.body.appendChild(node);
			...
			const onContextMenu = (event) => {
				if (event.target === null || typeof event.target.closest !== 'function') return;
