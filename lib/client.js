...
body[data-dshome-dark]{--ds-brand:#10B981;--ds-brand-deep:#059669;--ds-brand-mid:#34D399;--ds-brand-light:#6EE7B7;background:radial-gradient(1100px 650px at 8% -8%,#1a3870 0,transparent 62%),radial-gradient(950px 600px at 104% 10%,#2d5f9e 0,transparent 58%),radial-gradient(800px 600px at 50% 118%,#0d1f4a 0,transparent 60%),linear-gradient(180deg,#0a1a3a 0%,#0d1f4a 55%,#0a1a3a 100%);background-attachment:fixed}
/* Peak-hour hint: attribute toggled by the DSTT feature (Beijing time); the
   body-scoped variable override re-tints every var(--ds-brand*) consumer.
   Off-peak is green (safe); peak reverts to the classic blue. The light-mode
   peak background follows the base body rule through the variables. */
body[data-dshome-peak]{--ds-brand:#4D6BFE;--ds-brand-deep:#3A65C2;--ds-brand-mid:#4176E6;--ds-brand-light:#73A3D2}
body[data-dshome-dark][data-dshome-peak]{--ds-brand:#5D79FF;--ds-brand-deep:#4D6BFE;--ds-brand-mid:#7A96FF;--ds-brand-light:#9DB9FF;background:radial-gradient(1100px 650px at 8% -8%,#1a2f66 0,transparent 62%),radial-gradient(950px 600px at 104% 10%,#2d4a8c 0,transparent 58%),radial-gradient(800px 600px at 50% 118%,#0f1f4d 0,transparent 60%),linear-gradient(180deg,#0a1a3a 0%,#12234d 55%,#0a1a3a 100%)}
...
.dshome-switch{position:relative;flex:none;width:40px;height:22px;border-radius:999px;border:none;cursor:pointer;padding:0;background:color-mix(in srgb,var(--ds-brand) 25%,transparent);transition:background .18s ease}
.dshome-switch::after{content:"";position:absolute;top:2px;left:2px;width:18px;height:18px;border-radius:50%;background:#fff;box-shadow:0 1px 3px rgba(0,0,0,.25);transition:transform .18s ease}
.dshome-switch[aria-checked=true]{background:linear-gradient(135deg,var(--ds-brand),var(--ds-brand-mid))}
.dshome-switch[aria-checked=true]::after{transform:translateX(18px)}
.dshome-switch:disabled{opacity:.5;cursor:default}
.dshome-switch:focus-visible{outline:2px solid var(--dsw-alias-state-business-primary);outline-offset:2px}
body[data-dshome-dark] .dshome-switch{background:color-mix(in srgb,var(--ds-brand) 25%,transparent)}
...
`;
		...
		const ACTIONS_CSS = `
.dshome-locale{display:inline-flex;align-items:center;height:28px;border:1px solid color-mix(in srgb,var(--ds-brand) 25%,transparent);border-radius:999px;background:color-mix(in srgb,var(--ds-brand) 8%,transparent);overflow:hidden;flex:none}
.dshome-locale button{height:28px;padding:0 11px;border:none;background:transparent;color:var(--ds-brand);font-size:12px;font-weight:500;cursor:pointer;font-family:inherit;line-height:28px;transition:background .15s ease,color .15s ease}
.dshome-locale button.dshome-on{background:linear-gradient(135deg,var(--ds-brand),var(--ds-brand-mid));color:#fff}
...
` },
			// Sidebar glass root, brand, new-session button.
...
` },
			// Glass cards with spinning gradient border.
...
` }
		...
		*/
		...
		function startParticles() {
			...
			const isPeak = () => document.body.hasAttribute('data-dshome-peak');
			let running = true;
			let rafId = 0;
			let W = 0;
			let H = 0;
			...
			function palette() {
				if (isPeak()) {
					return isDark()
						? ['#5d79ff', '#4d6bfe', '#8fa8ff', '#ffffff', '#9db9ff']
						: ['#4d6bfe', '#73a3d2', '#9db9ff', '#ffffff'];
				}
				return isDark()
					? ['#34d399', '#059669', '#6ee7b7', '#a7f3d0', '#ffffff']
					: ['#10b981', '#6ee7b7', '#34d399', '#ffffff'];
			}
			...
			function step() {
				if (!running) return;
				g.clearRect(0, 0, W, H);
				const linkBase = isPeak()
					? (isDark() ? 'rgba(93,121,255,' : 'rgba(77,107,254,')
					: (isDark() ? 'rgba(52,211,153,' : 'rgba(16,185,129,');
		...
		* Falls back to workspaces.openPath when the RPC channel is unavailable.
		*/
				...
				item.addEventListener('click', (event) => {
					...
					const fallback = () => {
						try {
							...
							console.warn('[deepseek-style-theme] open workspace failed:', error);
						}
					};
					if (rpc !== undefined && typeof rpc.call === 'function') {
						Promise.resolve(rpc.call('/dshome-open-workspace', 'dshome/explorer.open', { path: path }))
							.then((result) => {
								if (result === undefined || result === null || result.ok !== true) fallback();
							})
							.catch(() => fallback());
					} else {
						fallback();
		...
		*/
		...
		// ── DSTT (DeepSeekStyleTheme) settings + peak-hour hint ───────────────
		// The host half owns the durable `deepseek-style-theme` settings
		// namespace; the browser half reads/writes one `mode` enum (auto =
		// time-sliced green/blue, blue = always blue, green = always green)
		// over this plugin's private loopback channel — the settings RPC
		// surface only serves the core's allowlisted namespaces. A shared
		// in-memory mirror feeds the event-chain clock, which checks the
		// Beijing time once, then schedules exactly the next boundary switch —
		// blue while inside a peak window (delta to the window end), green
		// outside (delta to the next window start). One-minute granularity
		// (ceiling) per the user's rule.
		...
		* plugin's own loopback channel instead; every reader (the peak-hour
		* driver, the settings tab) subscribes to this mirror.
		*/
		const dsttState = { mode: "auto", listeners: new Set() };
		...
		async function dsttSync(ctx) {
			...
			if (connection === undefined || connection === null || connection.rpc === undefined || typeof connection.rpc.call !== 'function') return;
			try {
				const result = await connection.rpc.call('/dshome-open-workspace', DSTT_GET, {});
		...
		async function dsttSetMode(ctx, mode) {
			...
			const connection = ctx.get('connection');
			if (connection === undefined || connection === null || connection.rpc === undefined || typeof connection.rpc.call !== 'function') return false;
			try {
				const result = await connection.rpc.call('/dshome-open-workspace', DSTT_SET, { mode });
				if (result !== undefined && result !== null && result.ok === true && DSTT_MODES.includes(result.value.mode)) {
					dsttState.mode = result.value.mode;
					dsttNotify();
					return true;
		...
		* field). One-minute granularity: 12:59:17 reads as 13:00 → blue, and the
		* moment the wall clock hits 14:00 the pink takes over; 13:59:30 already
		* counts as 14:00 (inside the window) because it rounds up.
		*/
		...
		/** Whether a rounded Beijing minute falls inside a peak window. */
		...
		/** Product-token layer applied while the peak-hour hint is active: the
		* classic blue (off-peak is green). */
		const PEAK_TOKENS = {
			'--dsw-alias-brand-primary': { light: '#4d6bfe', dark: '#5d79ff' },
			'--dsw-alias-state-business-primary': { light: '#4d6bfe', dark: '#5d79ff' }
		};

		/**
		* Peak-hour driver — event chain, no polling. On start it checks the
		* Beijing clock once, then schedules exactly the next boundary switch:
		* inside a window → pink now, record the delta to the window end, flip
		* back to blue when it elapses; outside → blue, flip to pink at the next
		* window start. Toggling the DSTT switch re-anchors immediately; closing
		* the feature tears every timer and token layer down. A
		* `visibilitychange` re-anchors the chain from the live clock in case a
		* throttled background tab drifted.
		*/
		function watchPeakHour(ctx, theme) {
			let peakDispose = null;
			let chainTimer = 0;
			let stopped = false;
			let unsubscribe = null;

			const currentMode = () => dsttGetMode();

			const applyPeak = (peak) => {
				const before = document.body.hasAttribute('data-dshome-peak');
				document.body.toggleAttribute('data-dshome-peak', peak);
				if (before !== peak) {
					console.info('[' + PLUGIN_ID + '] peak state -> ' + (peak ? 'blue' : 'green') + ' @ ' + beijingClock(beijingNow()));
				}
				if (peakDispose !== null) {
					peakDispose();
					peakDispose = null;
				}
				if (peak && theme !== undefined && theme !== null && typeof theme.overrideTokens === 'function') {
					try {
						peakDispose = theme.overrideTokens('deepseek-style-theme-peak', PEAK_TOKENS);
					} catch (error) {
						peakDispose = null;
					}
				}
			...
			const anchor = () => {
				...
				if (mode === "blue") {
					// Always blue: pin the peak attribute/token layer on.
					applyPeak(true);
					return;
				}
				if (mode === "green") {
					// Always green: no peak distinction at all.
					applyPeak(false);
					return;
				}
			...
			return () => {
				...
				if (unsubscribe !== null) unsubscribe();
				if (peakDispose !== null) peakDispose();
				document.removeEventListener('visibilitychange', onVisibility);
			};
		}

		/**
		* DSTT settings tab (title: DSTT). Renders the peak-hour pink toggle plus
		* a live status line. Reads/writes the durable namespace through the bound
		* settings scope; every subscription and timer is owned by the component.
		*/
		function DsttSection(props) {
			const dstt = props.dstt;
			const locale = props.locale;
			const [mode, setMode] = React.useState(() => (dstt === undefined ? 'auto' : dstt.mode()));
			const [active, setActive] = React.useState('zh');
			...
			const status = mode === 'blue'
				? (isZh ? '固定蓝色模式' : 'Fixed blue')
				: mode === 'green'
					? (isZh ? '固定绿色模式' : 'Fixed green')
					: peakNow
						? (isZh ? '高峰时段 · 主题色为蓝色' + (deltaText === '' ? '' : ' · ' + deltaText) : 'Peak hours · blue theme' + (deltaText === '' ? '' : ' · ' + deltaText))
						: (isZh ? '非高峰时段 · 绿色' : 'Off-peak hours · green');
			...
			return React.createElement('div', { className: 'dshome-dstt' },
				...
				React.createElement('p', null, isZh
					? '非高峰时段主题为绿色（放心使用）；高峰时段（北京时间 9:00–12:00、14:00–18:00）自动切换为蓝色提示。'
					: 'Off-peak the theme is green (safe to use); during peak hours (Beijing time 9:00–12:00, 14:00–18:00) it automatically switches to blue as a hint.'),
				React.createElement('div', { className: 'dshome-dstt-row' },
					React.createElement('div', { className: 'dshome-dstt-row-body' },
						...
						React.createElement('div', { className: 'dshome-dstt-row-hint' }, isZh
							? '分时段：高峰蓝、非高峰绿；默认蓝 / 默认绿：始终固定，无高峰区分'
							: 'Auto: blue at peak, green off-peak; fixed blue / green: always, no peak distinction')),
					React.createElement('div', { className: 'dshome-dstt-seg', role: 'radiogroup', 'aria-label': isZh ? '主题模式' : 'Theme mode' },
						DSTT_MODES.map((m) => React.createElement('button', {
							...
							onClick: () => pickMode(m)
						}, m === 'auto' ? (isZh ? '分时段' : 'Auto') : m === 'blue' ? (isZh ? '默认蓝' : 'Blue') : (isZh ? '默认绿' : 'Green'))))),
				React.createElement('div', { className: 'dshome-dstt-status', 'data-peak': mode === 'auto' && peakNow }, status));
		}
		...
		function apply(ctx) {
			...
			// namespaces). Pull it once, then let the peak-hour driver and the
			// settings tab share the in-memory mirror.
			ctx.effect(() => {
				dsttSync(ctx);
			}, 'deepseek-style-theme: dstt sync');
			ctx.effect(() => watchPeakHour(ctx, theme), 'deepseek-style-theme: peak hour');
