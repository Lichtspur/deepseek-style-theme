		...
		function watchWorkspaceMenu(ctx) {
			...
			const injectItem = (menu) => {
				...
				item.addEventListener('click', (event) => {
					...
					// dsh ≤0.1.1 exposed `workspaces.openPath`; 0.1.5 removed it, so the
					// private host channel is the only reveal path there. Fall back to
					// openPath only when it still exists, and otherwise say out loud why
					// nothing happened instead of throwing into the void.
						...
						Promise.resolve(rpc.call('/dshome-open-workspace', 'dshome/explorer.open', { path: path }))
							.then((result) => {
								...
								const detail = result !== undefined && result !== null && result.error !== undefined
									...
									: 'no result';
								fallback('rpc returned ' + detail);
							})
							.catch((error) => {
								fallback('rpc call threw: ' + (error instanceof Error ? error.message : String(error)));
							});
					} else {
						fallback('no connection.rpc on this page');
					}
					try {
