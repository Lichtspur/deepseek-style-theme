 ...
 */
async function dsttWrite(ctx, mode, fluidBrush, glassStyle, composerRefraction, backdropBlur, backgroundMode, customBackground) {
	...
	const handler = async (endpoint, payload) => {
		try {
			...
			if (endpoint === DSTT_SET) {
				...
				return await dsttWrite(ctx, payloadObject.mode, payloadObject.fluidBrush, payloadObject.glassStyle, payloadObject.composerRefraction, payloadObject.backdropBlur, payloadObject.backgroundMode, payloadObject.customBackground);
			}
