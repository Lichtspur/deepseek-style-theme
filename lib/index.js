...
function openFileGeneric(path) {
	switch (process.platform) {
		case "win32":
			return runOpener("powershell.exe", ["-NoProfile", "-Command", `Start-Process -LiteralPath ${powershellLiteral(path)}`]);
		case "darwin":
			return runOpener("open", [path]);
		case "linux":
			return runOpener("xdg-open", [path]);
...
function dsttRead(ctx) {
	...
	return { ok: true, value: { mode: DSTT_MODES.includes(stored) ? stored : DSTT_MODE_DEFAULT } };
}
