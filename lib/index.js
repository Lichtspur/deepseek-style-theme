...
function apply(ctx) {
	...
	// Property access (ctx.connection) rather than ctx.get("connection") is
	// load-bearing: cordis' context proxy binds the *reading* ctx to the service
	// it hands out, while ctx.get() returns the raw service whose `ctx` is still
	// the provider's. `connection.rpc.handle` registers its route through
	// `owner.webServer`, so with a raw service the guard evaluates the provider's
	// ctx and throws `cannot get property "webServer" without inject` no matter
	// what this plugin declares in its own inject list.
