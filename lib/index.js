...
// How wide the composer's edge refraction is under the liquid recipe. `narrow`
// is a map built for that card's own filter-region aspect, so the band is about
// 4px and the same on all four edges; `origin` is upstream's map, which on a
// short card gives a ~20px band top and bottom and almost nothing left and
// right; `off` drops the dispersion from the card entirely.
const COMPOSER_REFRACTIONS = ["narrow", "origin", "off"];
 ...
 * `fluidBrush`, `glassStyle` and `composerRefraction` are all optional: when they
 * carry a valid value the same write also sets them, so the settings panel can
 * persist a choice without a second round trip. None of them is mandatory the
 * way `mode` is — an older client that only knows about the mode keeps working.
 */
async function dsttWrite(ctx, mode, fluidBrush, glassStyle, composerRefraction) {
...
/** The stored composer-refraction width, defaulting to the purpose-built map. */
	...
	const handler = async (endpoint, payload) => {
		try {
			...
			if (endpoint === DSTT_SET) {
				...
				return await dsttWrite(ctx, payloadObject.mode, payloadObject.fluidBrush, payloadObject.glassStyle, payloadObject.composerRefraction);
			}
