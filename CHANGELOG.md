...
### 变更
- **对话框边缘折射改为可调，并新增一项设置**（`composerRefraction`，默认 `narrow`）。三选一：
  - **窄（均匀）**＝本次新做的位移图，四边折射带各约 4px；
  - **原样**＝上游那张 `400x92` 的图，短卡片上上下约 20px、左右几乎没有；
  - **关闭**＝对话框不上折射。
  设置页 DSTT 里新增一行「对话框边缘倒影」，与「玻璃风格」同形。
...
### 为什么"窄"版要换一张图
滤镜区域是元素宽度的 124%、高度的 156%，位移图用 `preserveAspectRatio="none"` 拉伸过去。于是图内缩 `e` 个单位，落到屏幕上横向是 `(e/w)*1.24W`、纵向是 `(e/h)*1.56H`——**只有图的宽高比等于滤镜区域的宽高比时两边才一样宽**。上游那张 `400x92`（4.3:1）对上一张 764×112 卡片的区域（947×175，5.4:1），纵向斜坡自然比横向厚好几倍，这就是"上下有、左右没有"的来源。新图取 `400x74`（5.4:1）并给两轴同一个**绝对**内缩 `0.8`，配 `blur(0.9)`；按 `k = 400/947 px/map unit` 算，屏上约 `(0.8+0.9)/0.4224 ≈ 4px`。

上游那两张图的默认参数（内缩 `min(w,h)*0.035`、`blur(11)`）原样保留，所以**白磨砂的地图逐字节未变**。
...
### 未验证
- **本轮所有视觉改动都没能在无头浏览器里复核**：Chrome 起得来（`/json/version` 有响应），但 CDP 的 WebSocket 一直不回应 `Page.enable`，新起的实例一律如此。因此"约 4px、四边均匀"是**按几何算出来的**，不是看出来的；下一次能截图时需要补一张确认。
