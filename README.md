...
- **时段提示（DSTT）**：主题模式三选一——**分时段**（高峰蓝、非高峰绿）/ **默认蓝**（始终蓝）/ **默认绿**（始终绿）；高峰窗口北京时间 9:00–12:00、14:00–18:00

## DSTT（DeepSeekStyleTheme）设置

设置页新增 **DSTT** 选项卡，提供一行三选「主题模式」（`deepseek-style-theme` 段的 `mode` 字段，持久化到 `settings.yaml`）：
- **分时段**（默认）：非高峰 = 绿色（放心用），高峰 = 蓝色（提示）；
- **默认蓝**：始终蓝色，无高峰区分；
- **默认绿**：始终绿色，无高峰区分。
...
- 高峰窗口：9:00–12:00、14:00–18:00。分时段模式下高峰自动变蓝（事件链计时：启动/切换时监测一次，记录距高峰结束的差值并计时还原绿色；非高峰则计时到下一高峰开始变蓝），退出高峰自动还原。
...
```bash
dsh plugin --profile web add ./releases/dsh-external-dsh-deepseek-style-theme-1.37.3.tgz
```
