...
- **时段提示（DSTT）**：主题模式四选一——**峰谷红蓝**（高峰鲜红、谷时段蓝）/ **峰谷红绿**（高峰鲜红、谷时段绿）/ **常态绿** / **常态蓝**；高峰窗口北京时间 9:00–12:00、14:00–18:00，高峰色为鲜红 `#F5222D`
...
- 高峰窗口：9:00–12:00、14:00–18:00。峰谷模式下高峰自动变鲜红（事件链计时：启动/切换时监测一次，记录距高峰结束的差值并计时还原谷色；非高峰则计时到下一高峰开始变红），退出高峰自动还原。
...
```bash
dsh plugin --profile web add ./releases/dsh-external-dsh-deepseek-style-theme-1.38.0.tgz
```
